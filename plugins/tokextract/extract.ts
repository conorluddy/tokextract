#!/usr/bin/env node
/**
 * extract.ts — Tokextract CLI entry point
 *
 * Subcommands:
...
const TOKEXTRACT_VERSION = "1.0.0-slice3";
...
program
  .name("extract")
  .description("Tokextract — reverse-engineer a SwiftUI design system to DTCG tokens")
  .version(TOKEXTRACT_VERSION);
...
program
  ...
  .option("--output <dir>", "Output directory (default: <path>/.tokextract-out)")
    ...
    async (opts: {
      ...
      await runParse({
        ...
        outputDir: opts.output
          ...
          : path.resolve(opts.path, ".tokextract-out"),
        noLlm: !opts.llm,
...
async function runParse(options: ParseOptions): Promise<void> {
  ...
  log(verbose, `Tokextract ${TOKEXTRACT_VERSION} — parse`);
  ...
  const tokextractStateDir = path.join(outputDir, ".tokextract");
  fs.mkdirSync(tokextractStateDir, { recursive: true });

  const findingsFile: FindingsFile = {
    tokextractVersion: TOKEXTRACT_VERSION,
    targetRepo: repoPath,
    extractedAt: new Date().toISOString(),
    findings: [...allFindings, ...hexFindings],
  };

  const findingsPath = path.join(tokextractStateDir, "findings.raw.json");
  ...
  const clusterPath = path.join(tokextractStateDir, "clusters.json");
  fs.writeFileSync(
    clusterPath,
    `${JSON.stringify(
  ...
  const numericClusterPath = path.join(tokextractStateDir, "numericClusters.json");
  fs.writeFileSync(
    numericClusterPath,
  ...
  // Write drift report
  const driftPath = path.join(tokextractStateDir, "drift.json");
  ...
  const metaPath = path.join(tokextractStateDir, "meta.json");
  fs.writeFileSync(
    metaPath,
  ...
  if (!noLlm) {
    console.log("\nNext: read .tokextract/llm-tasks.json and run pending LLM tasks.");
...
async function runPlanHarmonize(options: PlanHarmonizeOptions): Promise<void> {
  ...
  const stateDir = path.join(outputDir, ".tokextract");
...
async function runEmit(options: EmitOptions): Promise<void> {
  ...
  log(verbose, `Tokextract ${TOKEXTRACT_VERSION} — emit`);
  log(verbose, `  output: ${outputDir}`);

  // Read findings
  const findingsPath = path.join(outputDir, ".tokextract", "findings.raw.json");
  ...
  const clusterPath = path.join(outputDir, ".tokextract", "clusters.json");
  ...
  const numericClusterPath = path.join(outputDir, ".tokextract", "numericClusters.json");
  ...
  const driftPath = path.join(outputDir, ".tokextract", "drift.json");
  ...
  const metaPath = path.join(outputDir, ".tokextract", "meta.json");
  ...
  const llmOutDir = path.join(outputDir, ".tokextract", "llm-out");
  const harmonizeRecommendations = loadHarmonizeRecommendations(llmOutDir);

  // === Stage 6b: Diff against previous tokens.json ===
  const previousTokensPath = path.join(outputDir, ".tokextract", "previous", "tokens.json");
...
async function runFinalize(options: FinalizeOptions): Promise<void> {
  ...
  log(verbose, `Tokextract ${TOKEXTRACT_VERSION} — finalize`);
...
function hasNarrateOutput(outputDir: string): boolean {
  return fs.existsSync(path.join(outputDir, ".tokextract", "llm-out", "narrate.md"));
}
