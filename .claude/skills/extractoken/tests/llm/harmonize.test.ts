...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "extractoken-harmonize-test-"));
});
...
describe("harmonize — manifest task structure", () => {
  ...
  it("sets responsePath under <outputDir>/.extractoken/llm-out/", () => {
    const llmTasks: LlmTask[] = [];
    ...
    const task = llmTasks[0];
    const expectedPrefix = path.join(tmpDir, ".extractoken", "llm-out");
    expect(task?.responsePath).toContain(expectedPrefix);
    expect(task?.responsePath).toContain("mapping.harmonize.json");
  });

  it("sets promptPath under <outputDir>/.extractoken/prompts/", () => {
    const llmTasks: LlmTask[] = [];
    ...
    const task = llmTasks[0];
    const expectedPrefix = path.join(tmpDir, ".extractoken", "prompts");
  ...
  it("marks task as done if mapping.harmonize.json already exists and is a valid array", () => {
    ...
    const llmOutDir = path.join(tmpDir, ".extractoken", "llm-out");
