...
export interface NormalizeOptions {
  ...
  readonly extractokenSkillDir: string; // Path to ~/.claude/skills/extractoken/
...
export function writeNormalizeManifest(
  ...
  const manifestPath = path.join(options.outputDir, ".extractoken", "llm-tasks.json");
  const findingsPath = path.join(options.outputDir, ".extractoken", "findings.raw.json");
  ...
  for (const category of options.categories) {
    ...
    for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
      ...
      const responseSchema = path.join(options.extractokenSkillDir, "schemas", "mapping.json");
...
function buildNormalizePrompt(opts: PromptOptions): string {
  ...
  return `# Extractoken — ${categoryLabel} Token Normalization${chunkLabel}
