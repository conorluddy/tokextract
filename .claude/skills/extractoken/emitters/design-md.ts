...
${options.isStub ? "_[Stub — LLM narration pass not run. Re-run without `--no-llm` for brand-intent prose.]_\n\n" : ""}${options.appName} design system extracted by Extractoken. ${totalTokens} tokens across ${categoryCount} categor${categoryCount === 1 ? "y" : "ies"}. See \`tokens.json\` for the complete token inventory and \`audit.md\` for actionable harmonization recommendations.`;
}
