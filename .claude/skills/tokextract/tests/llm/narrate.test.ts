...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "tokextract-narrate-test-"));
});
...
describe("narrate — manifest task structure", () => {
  ...
  it("sets responsePath to <outputDir>/DESIGN.md — NOT under .tokextract/llm-out/", () => {
    const llmTasks: LlmTask[] = [];
    ...
    // Must not be buried in the internal .tokextract dir
    expect(task?.responsePath).not.toContain(".tokextract");
  });
